...
A problem in the implementation we suggested in this chapter could be caused by our treatment of the contour counter, represented by the graph variable \isa{Vcnt}. Every node reads it, and almost all nodes increase it. This could cause a lot of unwanted dependencies between the nodes, rendering the slicing analysis worthless.
...
A similar problem is posed by the binding environment in the graph variable \isa{V\isasymbeta}, which is also written by each lambda expression and used by each call expression. Again, dedicated nodes could avoid having \isa{V\isasymbeta} in the \isa{Def} sets of lambda expressions.
